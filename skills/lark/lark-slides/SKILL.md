...
飞书幻灯片通常是用户自己的内容资源。**严格按 `lark-shared` 的全局默认执行**——
所有 slides 命令都显式带 `--as user`，不要依赖 CLI 的隐式默认（CLI 底层默认是 `bot`）。

- **`--as user`（默认 / 唯一推荐）**：以当前登录用户身份创建、读取、管理演示文稿。
  如果未登录，按 `lark-shared` 的两段式流程完成
  `lark-cli auth login --no-wait --json --domain all`（一次性全域授权，
  **不要**写 `--domain slides` 之类的窄域）。
- **`--as bot`**：仅在用户明确要求以应用身份操作，或需要让 bot 持有/创建资源
  时使用；并额外确认 bot 是否真的有目标演示文稿的访问权限。
