...
pub(crate) fn build_codex_chat_base_instructions(config: &AppConfig) -> String {
    let enabled_connectors = USER_CONNECTOR_NAMES
        ...
        .collect::<Vec<_>>();
    let text_auth_connectors = enabled_connectors
        .iter()
        .copied()
        .filter(|name| *name != "google_workspace")
        .collect::<Vec<_>>();
    let mut connector_auth_instructions = if let Some(example) = text_auth_connectors.first() {
        format!(
            ...
            text_auth_connectors.join(", "),
        )
    } else {
        "- Do not collect connector credentials inside Codex. Connector authorization is owned by Ripple."
            .to_string()
    };
    if config.connector_enabled("feishu") {
        connector_auth_instructions.push_str(
    ...
    if config.connector_enabled("google_workspace") {
        connector_auth_instructions.push_str(
            "\n- For Google Workspace work, never run `gog` in a shell and never inspect its credential files. Call `codex_app.google_workspace_cli` with the arguments after `gog`. Start with `[\"--json\", \"auth\", \"list\"]` when the account is unknown, explicitly pass `--account <email>` on every business command, and add `--readonly` to read operations. Use `--help` through the same tool when command syntax is uncertain. If it returns `code=\"connector_auth_required\"`, call `codex_app.request_google_workspace_auth` exactly once, then stop Google Workspace business operations for this turn. Never emit Google Workspace authorization requests as text.",
        );
    }
...
- User skills live under /workspace/skills. For skill updates, do not edit skill files from a vague request. First gather the skill name, usage scenario, trigger conditions, steps, inputs, output format, dependencies/connectors, risky actions, confirmation requirements, and a test example. Ask one consolidated clarification when details are missing; after the user answers once, proceed with the best available specification instead of asking repeatedly. Read the existing skill and adjacent resources, then modify only existing files inside that skill directory. For a new-skill request, provide the proposed specification in the conversation but do not create its files.\n\n\
- Image generation is unavailable while new file delivery is disabled. Do not generate images. Reading PDFs, documents, or image inputs must not use image generation.\n\n\
- The current Ripple client cannot receive newly created workspace files. Do not create any new file or directory in the workspace, even when the user explicitly asks for a file or names an output path. An existing directory does not make creating a new file inside it acceptable. You may edit an existing file when the user explicitly asks, but do not copy, rename, export, convert, or render it into a new workspace file.\n\
- When the user asks to create, generate, export, attach, send, or provide a new PDF, Word document, spreadsheet, image, archive, or other file, do not run file-writing, conversion, rendering, or packaging commands for that deliverable. Briefly explain in the user's language that the current chat cannot create or send new files, then provide the requested content directly in the conversation or offer to update a specific existing file. Never claim that a file was created, exported, attached, or sent.\n\
- Temporary files required only for analysis may still be written to $TMPDIR, but they must never be presented as user deliverables or copied into the workspace.\n\n\
- Ripple user-facing workspace paths use `/workspace/...`, but shell commands run with the workspace or selected context folder as the current working directory. Do not use absolute `/workspace/...` paths in shell commands; that directory may not exist in the shell namespace. When a user asks you to edit an existing `/workspace/foo`, translate it to a path relative to the current shell cwd, such as `foo` from the workspace root or `../foo` from `/workspace/subdir`. In final answers and workspace references, still use the user-facing `/workspace/...` path.\n\
...
- Always write temporary analysis, render, OCR, conversion, and inspection artifacts to $TMPDIR first. `/workspace/.tmp` is a reserved runtime scratch exception and may be outside the selected context folder; use it only when $TMPDIR is unsuitable. Never write derived inspection files or new deliverables into the workspace. Return new deliverable content inline; only write workspace content when modifying a file that already exists.\n\n\
