"""NotionTokenSet — 把用户在对话里贴出的 Notion Integration Token 绑定到当前 session

设计理念：
  * **不依赖前端 UI**：用户直接在对话框里贴 token，模型识别后调本工具
    完成绑定。这样后端能给任意前端（CLI、Web、SDK）共用，无需各自实现
    一个 "token 输入卡片"。
  * **per-session 严格隔离**：token 只写到 `session_dir/notion.json`，
    不污染全局 / 其他 session；session sandbox 本身是 per-user 隔离
    且严格保密，不会泄露给其他用户。
  * **不回显 token**：工具的 result.data 里**绝不**包含 token 原文，
    只返回前 6 字符 + 长度，避免后续 turn 把 token 反复带进 LLM 上下文。
  * **不主动劝用户 Regenerate**：既然 session 是隔离保密的，绑定成功就
...
class NotionTokenSetTool(Tool):
    """把 Notion Integration Token 绑定到当前 session（per-session 隔离）"""

    def __init__(self):
        self.name = "NotionTokenSet"
        self.description = (
            "Bind a Notion Internal Integration Token to the current session "
            "(per-session, strictly isolated). Use this **immediately** after "
            "the user pastes their Notion token (typically `ntn_...` or "
            "`secret_...`) in the chat. After binding, retry the original `ntn` "
            "command — the sandbox will pick up the token via `NOTION_API_TOKEN` env.\n\n"
            "IMPORTANT:\n"
            ...
            "  or claim 'the token is exposed in conversation history'. The session "
            "  sandbox is per-user isolated with strict privacy measures; tokens "
            "  pasted here are not leaked to others. Only mention rotation if the "
    ...
    async def call(
        ...
        if not session_id:
            return ToolResult(
                data={
                    "ok": False,
                    "error": "当前上下文没有 sandbox_session_id，无法定位写入位置",
                }
            )

        try:
            write_notion_token(_sandbox_config, session_id, api_token)
            write_nsjail_config(_sandbox_config, session_id)
        except OSError as e:
            logger.error("session {} 写入 notion.json 失败: {}", session_id, e)
            return ToolResult(data={"ok": False, "error": f"写入失败: {e}"})

        masked = f"{api_token[:6]}...({len(api_token)} chars)"
        logger.info("session {} Notion token 已绑定 ({})", session_id, masked)
        ...
        return ToolResult(
            data={
                ...
                "next": (
                    "Token 已绑定到当前 session 的沙箱环境。请立刻重跑刚才被拦下的 `ntn` 命令，"
                    "直接继续业务即可。不要主动劝用户 Regenerate / rotate token —— "
                    "session sandbox 是 per-user 隔离且严格保密的，token 不会泄露给其他用户，"
                    "只有用户明确问起安全问题时才给建议。"
                ),
            }
        )
