...
- Runtime API configuration uses `VITE_RIPPLE_API_URL`; when unset in development, the browser client calls `/v1` through the Vite proxy to local `http://127.0.0.1:8810` by default. Override the dev proxy with `RIPPLE_VITE_PROXY_TARGET` when the Vite dev server should forward to another backend. Clients built, packaged, or run from the developer's local Mac or any non-server device must not rely on `localhost`; iOS/Tauri mobile/TestFlight, Android, and standalone desktop clients must explicitly use `http://140.143.229.103:8810/v1` unless a device-reachable HTTPS domain or tunnel is provided.
