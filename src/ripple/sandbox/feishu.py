...
2. per-user 凭证完全隔离：沙箱内 HOME=/workspace，lark-cli 的 app 配置
...
class _FeishuSetupState:
    """一个 user 正在进行中的 `lark-cli config init --new` 进程
...
def _get_feishu_credentials(config: SandboxConfig, user_id: str) -> tuple[str, str, str] | None:
    """读取 user 级飞书 app 凭证，返回 (app_id, app_secret, brand) 或 None"""
    feishu_file = config.feishu_config_file(user_id)
...
async def _start_feishu_setup(
    ...
    """在 user 沙箱内启动 `lark-cli config init --new`，提取 URL 后立即返回。
    ...
    _feishu_setup_states[user_id] = _FeishuSetupState(process=proc, url=url)
...
async def _check_feishu_setup(
    ...
    """
    state = _feishu_setup_states.get(user_id)
    if not state:
        return False, ""
    ...
    del _feishu_setup_states[user_id]

    if exit_code != 0:
        return False, f"config init --new 失败 (exit={exit_code})"

    if not config.has_lark_cli_config(user_id):
...
async def _inject_feishu_credentials(
    ...
    argv = build_nsjail_argv(config, user_id, inner_cmd)
...
async def ensure_lark_cli_config(
    ...
    lock = _lark_cli_config_locks.setdefault(user_id, asyncio.Lock())
    async with lock:
        if config.has_lark_cli_config(user_id):
            return True, ""
        ...
        if user_id in _feishu_setup_states:
            ok, msg = await _check_feishu_setup(config, user_id)
            if ok:
                return True, ""
            return False, msg
