...
lark-cli im +chat-list --sort-type ByActiveTimeDesc --as user

# Limit page size
lark-cli im +chat-list --page-size 50 --as user

# Pagination
lark-cli im +chat-list --page-token "xxx" --as user

# Drop muted chats (user identity only)
lark-cli im +chat-list --exclude-muted --as user

# JSON output
lark-cli im +chat-list --format json --as user

# Preview the request without executing it
lark-cli im +chat-list --dry-run --as user
```
...
```bash
lark-cli im +chat-list --sort-type ByActiveTimeDesc --page-size 10 --as user
```

### Scenario 2: List my non-muted chats sorted by activity

```bash
lark-cli im +chat-list --sort-type ByActiveTimeDesc --exclude-muted --as user
```
...
TOKEN=""
while :; do
  RESP=$(lark-cli im +chat-list --page-size 100 --page-token "$TOKEN" --format json --as user)
