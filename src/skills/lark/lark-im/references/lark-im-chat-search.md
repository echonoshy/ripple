...
lark-cli im +chat-search --query "project" --as user

# Restrict by search types
lark-cli im +chat-search --query "project" --search-types "private,public_joined" --as user

# Filter by member open_ids (with keyword)
lark-cli im +chat-search --query "project" --member-ids "ou_xxx,ou_yyy" --as user

# Search by member open_ids only
lark-cli im +chat-search --member-ids "ou_xxx,ou_yyy" --as user

# Only show chats you created or manage
lark-cli im +chat-search --query "project" --is-manager --as user

# Set page size
lark-cli im +chat-search --query "project" --page-size 10 --as user

# Pagination
lark-cli im +chat-search --query "project" --page-token "xxx" --as user

# JSON output
lark-cli im +chat-search --query "project" --format json --as user

# Preview the request without executing it
lark-cli im +chat-search --query "project" --dry-run --as user
```
...
```bash
lark-cli im +chat-search --query "design review" --as user
```

### Scenario 2: Search a chat and list recent messages

```bash
CHAT_ID=$(lark-cli im +chat-search --query "project" --format json --as user | jq -r '.data.chats[0].chat_id')
lark-cli im +chat-messages-list --chat-id "$CHAT_ID" --as user```

### Scenario 3: Search a chat and send a message

```bash
CHAT_ID=$(lark-cli im +chat-search --query "daily report" --format json --as user | jq -r '.data.chats[0].chat_id')
lark-cli im +messages-send --chat-id "$CHAT_ID" --text "Today's progress update" --as user
```
...
| Permission denied (99991679) with `--as user` | UAT is not authorized for `im:chat:read` | Ask the user to re-authorize Feishu in the Ripple conversation and mention `im:chat:read` |
