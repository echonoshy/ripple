#!/usr/bin/env bash
# 在项目内已安装的 lark-cli 版本之间切换，或列出全部可用版本。
#
# 用法:
#   ./scripts/use-feishu-cli.sh --list       # 列出所有版本，标记当前
#   ./scripts/use-feishu-cli.sh 1.0.13       # 切换 current -> v1.0.13
set -euo pipefail
...
if [[ ! -d "${INSTALL_ROOT}" ]]; then
  ...
  echo "  先运行: ./scripts/install-feishu-cli.sh <version>" >&2
  exit 1
fi
...
if [[ ! -x "${VERSION_DIR}/bin/lark-cli" ]]; then
  echo "✗ 未找到 v${VERSION}: ${VERSION_DIR}/bin/lark-cli 不存在或不可执行" >&2
  echo "  先运行: ./scripts/install-feishu-cli.sh ${VERSION}" >&2
  exit 1
fi
