...
export interface TaskSpecCreateInput {
  taskType: string;
  goal: string;
...
export interface TaskConfirmationCreateInput {
  title: string;
...
export interface TaskConfirmationRespondInput {
  decision: "allow" | "deny" | "accepted" | "rejected" | "yes" | "no";
  response?: unknown;
}
...
interface RawTaskSession {
  ...
  current_task_spec_id?: string | null;
...
function normalizeTaskSession(raw: RawTaskSession): TaskSessionInfo {
  return {
    sessionId: raw.session_id || raw.id || "",
    userId: raw.user_id || "",
    title: raw.title?.trim() || raw.goal?.trim() || "任务会话",
    status: raw.status || "waiting_user",
    sourceSurface: raw.source_surface ?? null,
    sourceId: raw.source_id ?? null,
    taskType: raw.task_type ?? null,
    goal: raw.goal ?? null,
    executor: raw.executor ?? null,
    latestMessage: raw.latest_message ?? null,
    needsUserAction: raw.needs_user_action === true,
    currentTaskSpecId: raw.current_task_spec_id ?? null,
    currentRunId: raw.current_run_id ?? null,
    latestRunId: raw.latest_run_id ?? null,
    createdAt: raw.created_at ?? null,
    updatedAt: raw.updated_at ?? null,
  };
}

function normalizeTaskSpec(raw: RawTaskSpec): TaskSpecInfo {
  return {
    taskSpecId: raw.task_spec_id || raw.id || "",
    sessionId: raw.session_id || "",
    userId: raw.user_id || "",
    taskType: raw.task_type || "other",
    goal: raw.goal ?? null,
    status: raw.status || "pending_confirm",
    requiredFields: raw.required_fields ?? null,
    sourceRefs: raw.source_refs ?? null,
    riskLevel: raw.risk_level ?? null,
    impactSummary: raw.impact_summary ?? null,
...
function normalizeTaskRun(raw: RawTaskRun): TaskRunInfo {
  return {
    runId: raw.run_id || raw.id || "",
    sessionId: raw.session_id || "",
    taskSpecId: raw.task_spec_id || "",
    userId: raw.user_id || "",
    status: raw.status || "in_progress",
    executor: raw.executor ?? null,
    externalRunId: raw.external_run_id ?? null,
    resultSummary: raw.result_summary ?? null,
...
function normalizeTaskSessionEvent(raw: RawTaskSessionEvent): TaskSessionEventInfo {
  return {
    eventId: raw.event_id || raw.id || "",
    sessionId: raw.session_id || "",
    userId: raw.user_id || "",
    eventType: raw.event_type || raw.type || "task_session_event",
    payload: raw.payload ?? null,
    createdAt: raw.created_at ?? null,
  };
}

function normalizeTaskConfirmation(raw: RawTaskConfirmation): TaskConfirmationInfo {
  return {
    confirmationId: raw.confirmation_id || raw.id || "",
    sessionId: raw.session_id || "",
    userId: raw.user_id || "",
    title: raw.title?.trim() || "需要确认",
    status: raw.status || "requested",
    confirmationType: raw.confirmation_type ?? null,
...
function normalizeTaskSessionDetail(raw: {
  task_session?: RawTaskSession;
  task_specs?: RawTaskSpec[];
  runs?: RawTaskRun[];
  events?: RawTaskSessionEvent[];
  confirmations?: RawTaskConfirmation[];
}): TaskSessionDetail {
  return {
    taskSession: normalizeTaskSession(raw.task_session || {}),
    taskSpecs: (raw.task_specs || []).map(normalizeTaskSpec),
    runs: (raw.runs || []).map(normalizeTaskRun),
    events: (raw.events || []).map(normalizeTaskSessionEvent),
    confirmations: (raw.confirmations || []).map(normalizeTaskConfirmation),
  };
}
...
function taskSpecInputForRequest(input: TaskSpecCreateInput): Record<string, unknown> {
  return {
...
function taskSessionInputForRequest(input: TaskSessionCreateInput): Record<string, unknown> {
  const body: Record<string, unknown> = {};
  if (input.title) body.title = input.title;
  if (input.sourceSurface) body.source_surface = input.sourceSurface;
  if (input.sourceId !== undefined) body.source_id = input.sourceId;
  if (input.taskType) body.task_type = input.taskType;
  if (input.goal) body.goal = input.goal;
  if (input.executor) body.executor = input.executor;
  if (input.initialMessage) body.initial_message = input.initialMessage;
  if (input.taskSpec) body.task_spec = taskSpecInputForRequest(input.taskSpec);
  return body;
}

function taskSessionUpdateInputForRequest(input: TaskSessionUpdateInput): Record<string, unknown> {
  const body: Record<string, unknown> = {};
  if ("title" in input) body.title = input.title;
  if ("status" in input) body.status = input.status;
  if ("latestMessage" in input) body.latest_message = input.latestMessage;
  if ("needsUserAction" in input) body.needs_user_action = input.needsUserAction;
  if ("currentTaskSpecId" in input) body.current_task_spec_id = input.currentTaskSpecId;
  if ("currentRunId" in input) body.current_run_id = input.currentRunId;
  return body;
}

function taskRunStartInputForRequest(input: TaskRunStartInput = {}): Record<string, unknown> {
  const body: Record<string, unknown> = {};
  if ("confirm" in input) body.confirm = input.confirm;
  if (input.executor) body.executor = input.executor;
  if (input.externalRunId) body.external_run_id = input.externalRunId;
  if (input.jobId) body.job_id = input.jobId;
  if (input.input) body.input = input.input;
  if (input.metadata) body.metadata = input.metadata;
  return body;
}

function taskRunUpdateInputForRequest(input: TaskRunUpdateInput): Record<string, unknown> {
  const body: Record<string, unknown> = {};
  if ("status" in input) body.status = input.status;
  if ("resultSummary" in input) body.result_summary = input.resultSummary;
  if ("failureReason" in input) body.failure_reason = input.failureReason;
  if ("cancellationReason" in input) body.cancellation_reason = input.cancellationReason;
  if (input.metadata) body.metadata = input.metadata;
  return body;
}

export async function fetchTaskSessions(): Promise<TaskSessionInfo[]> {
  return fetchPaginatedApiList<RawTaskSession, TaskSessionInfo>(
    `${API_URL}/task-sessions`,
    "task_sessions",
    normalizeTaskSession,
    "Failed to fetch task sessions"
  );
}

export async function fetchTaskSession(sessionId: string): Promise<TaskSessionDetail> {
  const res = await fetch(`${API_URL}/task-sessions/${encodeURIComponent(sessionId)}`, {
    headers: { ...authHeaders() },
  });
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to fetch task session (${res.status})`);
  }
  return normalizeTaskSessionDetail(await res.json());
}

export async function createTaskSession(input: TaskSessionCreateInput): Promise<TaskSessionDetail> {
  const res = await fetch(`${API_URL}/task-sessions`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
    body: JSON.stringify(taskSessionInputForRequest(input)),
  });
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to create task session (${res.status})`);
  }
  const data = (await res.json()) as {
    task_session?: RawTaskSession;
    task_spec?: RawTaskSpec | null;
  };
  return {
    taskSession: normalizeTaskSession(data.task_session || {}),
    taskSpecs: data.task_spec ? [normalizeTaskSpec(data.task_spec)] : [],
    runs: [],
    events: [],
    confirmations: [],
  };
...
export async function updateTaskSession(
  ...
  const res = await fetch(`${API_URL}/task-sessions/${encodeURIComponent(sessionId)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json", ...authHeaders() },
    body: JSON.stringify(taskSessionUpdateInputForRequest(input)),
  });
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to update task session (${res.status})`);
  }
  return normalizeTaskSessionDetail(await res.json());
}

export async function appendTaskSessionMessage(
  sessionId: string,
  input: { role?: "user" | "assistant" | "agent" | "system"; content: string; status?: string }
): Promise<{ taskSession: TaskSessionInfo; event: TaskSessionEventInfo }> {
  const res = await fetch(`${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/messages`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
    body: JSON.stringify({
  ...
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to append task session message (${res.status})`);
  }
  const data = (await res.json()) as { task_session?: RawTaskSession; event?: RawTaskSessionEvent };
  return {
    taskSession: normalizeTaskSession(data.task_session || {}),
    event: normalizeTaskSessionEvent(data.event || {}),
  };
}

export async function createTaskSpec(
  sessionId: string,
  input: TaskSpecCreateInput
): Promise<TaskSessionDetail> {
  const res = await fetch(`${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/task-specs`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
    body: JSON.stringify(taskSpecInputForRequest(input)),
  });
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to create task spec (${res.status})`);
  }
  return fetchTaskSession(sessionId);
}

export async function confirmTaskSpec(
  sessionId: string,
  taskSpecId: string,
  input: { startRun?: boolean } = {}
): Promise<TaskSessionDetail> {
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/task-specs/${encodeURIComponent(taskSpecId)}/confirm`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify({ start_run: input.startRun === true }),
    }
  );
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to confirm task spec (${res.status})`);
  }
  return fetchTaskSession(sessionId);
}
...
export async function startTaskRun(
  ...
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/task-specs/${encodeURIComponent(taskSpecId)}/runs`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify(taskRunStartInputForRequest(input)),
    }
  );
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to start task run (${res.status})`);
  }
  const data = (await res.json()) as { detail?: unknown };
  return normalizeTaskSessionDetail(isRecord(data.detail) ? data.detail : {});
}
...
export async function updateTaskRun(
  ...
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/runs/${encodeURIComponent(runId)}`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify(taskRunUpdateInputForRequest(input)),
    }
  );
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to update task run (${res.status})`);
  }
  const data = (await res.json()) as { detail?: unknown };
  return normalizeTaskSessionDetail(isRecord(data.detail) ? data.detail : {});
}
...
export async function cancelTaskRun(
  ...
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/runs/${encodeURIComponent(runId)}/cancel`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
  ...
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to cancel task run (${res.status})`);
  }
  const data = (await res.json()) as { detail?: unknown };
  return normalizeTaskSessionDetail(isRecord(data.detail) ? data.detail : {});
}
...
export async function createTaskConfirmation(
  sessionId: string,
  input: TaskConfirmationCreateInput
): Promise<TaskSessionDetail> {
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/confirmations`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify({
        title: input.title,
        confirmation_type: input.confirmationType || "allow_deny",
  ...
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to create confirmation (${res.status})`);
  }
  return fetchTaskSession(sessionId);
}

export async function respondTaskConfirmation(
  sessionId: string,
  confirmationId: string,
  input: TaskConfirmationRespondInput
): Promise<TaskSessionDetail> {
  const res = await fetch(
    `${API_URL}/task-sessions/${encodeURIComponent(sessionId)}/confirmations/${encodeURIComponent(confirmationId)}/respond`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify({
  ...
  if (!res.ok) {
    const detail = await responseDetail(res);
    throw new Error(detail || `Failed to respond confirmation (${res.status})`);
  }
  return fetchTaskSession(sessionId);
}
