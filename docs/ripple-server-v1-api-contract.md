...
- `POST /v1/sessions` 和 `PATCH /v1/sessions/:session_id` 会校验该路径必须在当前 user workspace 内且为目录。session 有 active run、pending approval、pending user input 或 context compaction 时，切换 context folder 返回 `409`。规范化后的路径实际变化时，Ripple 保留 session 消息，但解除原 `codex_thread_id`，下一轮创建绑定新 cwd 和 permission root 的 Codex thread；重复 PATCH 同一路径不会轮换 thread。
