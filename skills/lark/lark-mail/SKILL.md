...
邮箱是用户的个人资源，**严格按 `lark-shared` 的全局默认执行**——所有 mail 命令
都显式带 `--as user`（CLI 默认 `--as` 是 `bot`，不显式指定会走应用身份，结果
要么权限不足要么访问的是 bot 自己的"应用邮箱"，不是用户邮箱）。

- **`--as user`（默认 / 唯一推荐）**：以当前登录用户身份访问其邮箱。如果未登录，
  按 `lark-shared` 的两段式流程完成 `lark-cli auth login --no-wait --json --domain all`
  （一次性全域授权，**不要**写 `--domain mail` 之类的窄域）。
- **`--as bot`**：仅当用户明确要求"以应用身份操作邮箱"时才用，且需要在飞书
  开发者后台为应用开通相应权限。**bot 身份只能做读取类操作，所有写操作（发送、
  回复、转发、草稿编辑、撤回等）仅支持 user 身份。**

1. 所有邮件写操作（发送、回复、转发、草稿编辑） → 必须使用 `--as user`，未登录时按 `lark-shared` 两段式流程完成 `lark-cli auth login --no-wait --json --domain all`
2. 读取类操作（查看邮件、会话、收件箱列表等） → 默认 `--as user`；仅当用户明确要求"用应用身份代管理员批量读取"时才切到 `--as bot`，并确保应用已开通对应权限
