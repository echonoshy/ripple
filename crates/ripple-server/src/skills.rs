...
version: "9.9.9""#,
    ...
    #[test]
    fn viaim_product_support_is_exposed_as_regular_shared_skill() {
        let root = std::env::temp_dir().join(format!("ripple-skills-test-{}", Uuid::new_v4()));
        let skill = root.join("skills/viaim-product-support/SKILL.md");
        write_skill(
            &skill,
            r#"name: viaim-product-support
description: Use for viaim products, software downloads, technical support, after-sales service, and product support knowledge."#,
        );
        let reference = root.join("skills/viaim-product-support/references/ripple.md");
        ...
        assert!(entry
            .path
            .ends_with("skills/viaim-product-support/SKILL.md"));
        assert!(entry.description.contains("product support knowledge"));

        let rendered = render_skill_manifest(&config, None);
        assert!(rendered.contains("ripple:viaim-product-support"));
        assert!(rendered.contains("skills/viaim-product-support/SKILL.md"));
