...
仅 user 身份。需要 scope `contact:user:search`。所有示例都必须显式带 `--as user`；缺少 user 授权时应先走 connector 授权流程，不要切到 bot。
...
lark-cli contact +search-user --user-ids "ou_a,ou_b,ou_c" --format json --as user

# 多 filter 组合:同租户的、有企业邮箱的「王」姓员工
lark-cli contact +search-user --query "王" --exclude-external-users --has-enterprise-email --as user

# filter-only 枚举:列出所有"聊过天的离职同事"(无关键词)
lark-cli contact +search-user --has-chatted --left-organization --as user
```
...
lark-cli contact +search-user --queries "Alice,Bob" --has-chatted --as user

# 与 --query / --user-ids 互斥
lark-cli contact +search-user --queries "a" --query "b" --as user   # ❌ exit 2
```
