...
import {
  ...
  User, Loader2, ChevronDown, Brain, AlertTriangle, KeyRound, Trash2, Clock
} from "lucide-react";
import { Message, ToolCall, UsageInfo, Session } from "@/types";
import { createSession, sendChatMessage, fetchModels, getApiKey, setApiKey, clearApiKey, AuthError, fetchSessions, fetchSessionDetails, deleteSession } from "@/lib/api";
      ...
      if (details) {
        ...
        for (const msg of details.messages) {
          if (msg.role === 'user') {
            ...
            const contentStr = typeof msg.content === 'string' ? msg.content : (msg.content && Array.isArray(msg.content) ? msg.content.map((c: any) => c.text || '').join('') : '');
        ...
        <aside className="hidden md:flex flex-col w-72 glass-panel border-r border-white/40">
          ...
          <div className="px-4 pb-4 flex-1 overflow-y-auto">
            ...
            <div className="space-y-2 mb-6">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider px-2 mb-3">Recent Sessions</h2>
              {isLoadingSessions && sessions.length === 0 ? (
                <div className="flex justify-center p-4">
                  <Loader2 size={16} className="animate-spin text-slate-400" />
                </div>
              ) : sessions.length === 0 ? (
                <div className="text-xs text-slate-400 px-3 text-center py-2">No recent sessions</div>
              ) : (
                <div className="space-y-1">
                  {sessions.map(session => (
                    <div
                      ...
                      className={`group p-3 rounded-2xl border shadow-sm flex items-center justify-between cursor-pointer transition-all ${
                    ...
                    >
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all focus:outline-none"
          ...
          <header className="h-16 px-6 flex justify-between items-center glass-panel border-b border-white/40 z-20">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${
                sessionId ? 'bg-green-50 text-green-600 border-green-100/50' : 'bg-slate-50 text-slate-500 border-slate-200/50'
              }`}>
                <div className={`w-2 h-2 rounded-full ${sessionId ? 'bg-green-500 animate-pulse' : 'bg-slate-400'}`}></div>
                <span className="text-xs font-bold tracking-wide">{sessionId ? 'ACTIVE' : 'READY'}</span>
              </div>
            </div>
          </header>
            ...
            <div className="max-w-5xl mx-auto pointer-events-auto">
              <form onSubmit={handleSend} className="relative flex items-center">
                <input
                  ...
                  placeholder={isGenerating ? "Ripple is thinking..." : !sessionId ? "输入消息开始新对话..." : "Ask Ripple anything..."}
