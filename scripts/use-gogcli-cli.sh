#!/usr/bin/env bash
# 在项目内已安装的 gogcli (gog) 版本之间切换，或列出全部可用版本。
#
# 用法:
#   ./scripts/use-gogcli-cli.sh --list       # 列出所有版本，标记当前
#   ./scripts/use-gogcli-cli.sh 0.13.0       # 切换 current -> v0.13.0
set -euo pipefail
...
if [[ ! -d "${INSTALL_ROOT}" ]]; then
  ...
  echo "  先运行: ./scripts/install-gogcli-cli.sh <version>" >&2
  exit 1
fi
...
VERSION="${1#v}"  # 允许传 "0.13.0" 或 "v0.13.0"
VERSION_DIR="${INSTALL_ROOT}/v${VERSION}"

if [[ ! -x "${VERSION_DIR}/bin/gog" ]]; then
  echo "✗ 未找到 v${VERSION}: ${VERSION_DIR}/bin/gog 不存在或不可执行" >&2
  echo "  先运行: ./scripts/install-gogcli-cli.sh ${VERSION}" >&2
  exit 1
fi
